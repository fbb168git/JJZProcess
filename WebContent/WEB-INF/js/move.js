...
setupWebViewJavascriptBridge(function(bridge) {
	...
	$(function() {
		...
		$('.turn_bianming').click(function() {
			bridge.callHandler('pushViewController', {
				'type': 100,
				'url': 'https://api.accident.zhongchebaolian.com/chaifen/business-convenience-services/bm/bm.html?source=4'
			});
		});
		//进京证完成页面跳转便民服务
		...
		$('#turn_ul').click(function() {
			bridge.callHandler('pushViewController', {
				'type': 100,
				'url': 'https://api.accident.zhongchebaolian.com/chaifen/business-convenience-services/bm/ulPage.html?source=1'
			});
		});
		...
		$('#turn_tk').click(function() {
			bridge.callHandler('pushViewController', {
				'type': 100,
				'url': 'https://api.accident.zhongchebaolian.com/chaifen/business-convenience-services/bm/tkPage.html?source=1'
			});
		});
		$('#turn_xc').click(function() {
			bridge.callHandler('pushViewController', {
				'type': 100,
				'url': 'https://bjjj.zhongchebaolian.com/chaifen/business-convenience-services/sellCars.html?source=jjzsuccess'
			});
		});
		$('#turn_czb').click(function() {
			bridge.callHandler('pushViewController', {
				'type': 100,
				'url': 'http://m.mychebao.com/czhib_promote/temFriIndex.htm?id=378'
			});
		});
				...
				if (msg.rescode == 200) {
					...
					if (msg.count == 1) {
						...
						$('#homePageImg').click(function() {
							bridge.callHandler('pushViewController', {
								'type': 100,
								'url': 'http://m.banmachewu.com?tn=bjjj'
							});
						});
						/* 办理须知上侧广告 */
						...
						$('#adImg').click(function() {
							bridge.callHandler('pushViewController', {
								'type': 100,
								'url': 'http://insurert.union-ins.cn/product/wechat/insure/847685440834260991/traffic.html?channelId=188'
							});
						});
					}
				}
	...
	$('.yi').click(function(e) {
		...
		bridge.callHandler('chooseSimpleCarLocation', {
			'position': a
		}, function(response) {
			console.log('js调用客户端方法回调传参' + response);
		});
...
function addcard() {
	//alert('jinglaile');
	bridgewai.callHandler('addNewCar', {
		'defaultSimpleName': '冀'
	}, function(response) {
		console.log('js调用客户端方法回调传参' + response);
	});

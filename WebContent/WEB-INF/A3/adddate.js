...
mvAry[mvAry.length] = '    <table width="100%" border="0" cellpadding="0" cellspacing="1" style="font-size:0.26rem;height:40px;">'; 
mvAry[mvAry.length] = '      <tr>'; 
mvAry[mvAry.length] = '        <th align="left" width="1%" style="height:40px;"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:30px;height:40px;'; 
...
mvAry[mvAry.length] = '        <th align="center" width="98%" nowrap="nowrap"><select name="calendarYear" id="calendarYear" style="font-size:0.26rem ;height:40px;"></select><select name="calendarMonth" id="calendarMonth" style="font-size:12px;'; 
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏 “月” 
mvAry[mvAry.length] = '"></select></th>'; 
mvAry[mvAry.length] = '        <th align="right" width="1%" style="height:40px;"><input style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:30px;height:40px;'; 
if(calendar.DateMode > pickMode["month"]){mvAry[mvAry.length] = 'display:none;';}//pickMode 精确到年时隐藏 “月” 
mvAry[mvAry.length] ='" name="nextMonth" type="button" id="nextMonth" value="&gt;" /></th>'; 
mvAry[mvAry.length] = '      </tr>'; 
mvAry[mvAry.length] = '    </table>'; 
mvAry[mvAry.length] = '    <table id="calendarTable" width="100%" style="border:0px solid #CCCCCC;background-color:#FFFFFF;font-size:0.26rem;'; 
...
mvAry[mvAry.length] = '      <tr>'; 
for(var i = 0; i < 7; i++) { 
mvAry[mvAry.length] = '      <th style="height:30px;font-weight:normal;background-color:' + calendar.colors["tr_bg"] + ';color:' + calendar.colors["tr_word"] + ';">' + Calendar.language["weeks"][this.lang][i] + '</th>'; 
} 
mvAry[mvAry.length] = '      </tr>'; 
for(var i = 0; i < 6;i++){ 
...
for(var j = 0; j < 7; j++) { 
if (j == 0){ 
mvAry[mvAry.length] = ' <td style="height:30px;cursor:default;color:' + calendar.colors["sun_word"] + ';"></td>'; 
} else if(j == 6) { 
mvAry[mvAry.length] = ' <td style="height:30px;cursor:default;color:' + calendar.colors["sat_word"] + ';"></td>'; 
} else { 
mvAry[mvAry.length] = ' <td style="height:30px;cursor:default;"></td>'; 
} 
} 
mvAry[mvAry.length] = '    </tr>'; 
} 
...
mvAry[mvAry.length] = '      <div align="center" style="padding:10px;background-color:' + calendar.colors["input_bg"] + ';">'; 
mvAry[mvAry.length] = '        <input name="calendarClear" type="button" id="calendarClear" value="' + Calendar.language["clear"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:60px;height:30px;font-size:0.26rem;cursor:pointer;"/>'; 
...
mvAry[mvAry.length] = '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:60px;height:30px;font-size:0.26rem;cursor:pointer"/>'; 
mvAry[mvAry.length] = '        <input name="calendarClose" type="button" id="calendarClose" value="' + Calendar.language["close"][this.lang] + '" style="border: 1px solid ' + calendar.colors["input_border"] + ';background-color:' + calendar.colors["input_bg"] + ';width:60px;height:30px;font-size:0.26rem;cursor:pointer"/>'; 
...
this.panel.style.left = 0; //由寒羽枫 2007-02-11 修改 → 加入自定义偏移量 
...
{ 
var str = '<div id="calendarPanel" style="position:absolute;top:0!important;left:0!important;display: none;z-index:9; background-color: #FFFFFF;border: 1px solid #dcdcdc;width:100%;font-size:0.26rem;"></div>'; 
if(document.all) 
{ 
